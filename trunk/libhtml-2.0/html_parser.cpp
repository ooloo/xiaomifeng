...
int charset_detector(char *src, char *charsetbuff)
{
	...
	detectedCharset = detect_buffer_charset(detector, htmlBuffer, strlen(htmlBuffer));
	if (detectedCharset)
	{
...
void html_parser:: encode_conv()
{
	...
	cd = iconv_open((const char *)"gbk//ignore", (const char *)tree_->charset_);
	if (cd == (iconv_t)-1) {
	...
	else
	{
		...
		if ( content_[0] ) {
			...
			size_t ret;
			if((ret=iconv(cd, &pin, &inlen, &pout, &oleft)) != (size_t)(-1))
			{
				cflag = 0;
...
gettimeofday(&tpstart,NULL);
	...
	ret = tree_->html_tree_parse(url, page_, len);
...
timeuse = 0;
        ...
        tree_visitor_for_title *title_visitor = new tree_visitor_for_title(tree_);
...
timeuse = 0;
#endif
        tree_visitor_for_content *content_visitor = new tree_visitor_for_content(tree_);
        content_visitor->extract_content(&filter_len_, content_, max_content_len);
...
timeuse = 0;
	...
	UnescapeEntitiesForString(content_, strlen(content_), content_, max_content_len);
