...
int pick_a_time(char *text)
{
	...
	while (1)
	{
		...
		if(*q=='-' || *q=='/' || *q=='.')
		{
			...
			if(!match_hms(&q,&(ttm.tm_hour),&(ttm.tm_min),&(ttm.tm_sec)))
		...
		else
		{
			...
			{
				match_char(&q, ' ');
				if (!match_digits(&q,&(ttm.tm_mon)) || ttm.tm_mon<1 || ttm.tm_mon>12 || !match_2chars(&q,monthStr))
				{
					pstart = q;
					continue;
				}
...
*/
		...
		// 找到当前时间串所在的标签段落
		// 找开头
		for(paraLeft=p-1;*paraLeft && *paraLeft!='>' && *paraLeft!='<';paraLeft--)
			;
		...
		// 调整权值
		if((hint = strstr(paraLeft,"发布时间")) && hint < p && p - hint < HINT_DIST)
			curWeight+=100;

		if((hint = strstr(paraLeft,"发帖时间")) && hint < p && p - hint < HINT_DIST)
			curWeight+=100;

		if((hint = strstr(paraLeft,"发表")) && hint < p && p - hint < HINT_DIST)
			curWeight+=90;

		if((hint = strstr(paraLeft,"post")) && hint < p && p - hint < HINT_DIST)
			curWeight+=90;

		if((hint = strstr(paraLeft,"发行")) && hint < p && p - hint < HINT_DIST)
			curWeight-=50;

		if((hint = strstr(paraLeft,"现在")) && hint < p && p - hint < HINT_DIST)
			curWeight-=100;

		if((hint = strstr(paraLeft,"注册")) && hint < p && p - hint < HINT_DIST)
			curWeight-=200;

		if(((hint = strstr(paraLeft,"新闻来源")) || (hint = strstr(paraLeft,"稿源")))
			&& abs((int)(p - hint)) < HINT_DIST)
			curWeight+=90;

		if(((hint = strstr(paraLeft,"新闻网")) || (hint = strstr(paraLeft,"晨报"))
			|| (hint = strstr(paraLeft,"早报")) || (hint = strstr(paraLeft,"日报"))
			|| (hint = strstr(paraLeft,"晚报")) || (hint = strstr(paraLeft,"信息港")) || (hint = strstr(paraLeft,"在线")))
			&& abs((int)(p - hint)) < HINT_DIST)
			curWeight+=50;

		if(((hint = strstr(paraLeft,"编辑")) || (hint = strstr(paraLeft,"责编"))
			|| (hint = strstr(paraLeft,"责任编辑")) || (hint = strstr(paraLeft,"作者")))
			&& abs((int)(p - hint)) < HINT_DIST)
			curWeight+=50;

		if(((hint = strstr(paraLeft,"【")) || (hint = strstr(paraLeft,"】"))
			|| (hint = strstr(paraLeft,"〖")) || (hint = strstr(paraLeft,"〗")))
			&& abs((int)(p - hint)) < HINT_DIST)
			curWeight+=40;
		...
		if(((hint = strstr(paraLeft,"来源")) || (hint =strstr(paraLeft,"时间")))
			&& abs((int)(p - hint)) < HINT_DIST)
			curWeight+=10;

		if((hint = strstr(paraLeft,"http://")) && abs((int)(p - hint)) < HINT_DIST)
			curWeight+=10;

		if(((hint = strstr(paraLeft, "网")) || (hint = strstr(paraLeft, "报"))
			|| (hint = strstr(paraLeft, "社")) || (hint = strstr(paraLeft, "讯")))
			&& abs((int)(p - hint)) < HINT_DIST)
			curWeight+=10;
