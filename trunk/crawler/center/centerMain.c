...
int guide_recv(void *buf, int len)
{
	...
	while(htcp_recv(guideRecvSocket, buf, len) == -1)
	{
		...
		sleep(1);
		while((guideRecvSocket = htcp_connect(guideHost, g_basePort + 46)) == -1)
		{
			sleep(1);
...
int load_file(char *manualFile)
{
	...
	if(manualFile != NULL)
	{
		...
		{
			sscanf(src, "%d %s", &urlNode.bbsMysqlId, url);
			pthread_mutex_lock(&cache_mutex);
	...
	if(fp != NULL)
	{
		while(fgets(src, 256, fp) && filterNum < MAX_FILTER_NUM - 2)
		{
			if(get_domain(src, domain, 128) == -1)
				...
				assert(MD5(domain, strlen(domain), (unsigned char*)(signs+filterNum++)));
			printf("filter domain: %llu %s\n", signs[filterNum-1], domain);
		}
		fclose(fp);
	...
	while(fread(&domainKey, sizeof(KEY_T), 1, fp) == 1)
	{
		...
		if(filterNum > 0)
		{
			if(bsearch(&domainKey, signs, filterNum, sizeof(KEY_T), key_cmp) != NULL)
			{
