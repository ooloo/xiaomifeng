...
void save_html(char *filepath, URLNODE_T * urlNode, char *url, char *html)
{
    ...
    snprintf(filename, 256, "%s/html.%d.%02d.%02d.%02d.%02d.%02d.%u",
             filepath, ttm.tm_year + 1900, ttm.tm_mon + 1, ttm.tm_mday,
             ttm.tm_hour, ttm.tm_min, ttm.tm_sec, tmpNo++);
...
#ifdef BBS
    else if (saveFile.fileLen > 4 * 1024 * 1024
             || time(NULL) - saveTime > 60)
#else
    else if (saveFile.fileLen > 4 * 1024 * 1024
             || time(NULL) - saveTime > 1800)
#endif
    {
        fclose(saveFile.fp);
...
int news_url_filter(char *url)
{
    ...
    } else
    {
        ...
        else if ((t = strrchr(p, '.')) != NULL)
        {
            if (!strcasestr(t, "htm")
                && !strcasestr(t, "jsp")
                && !strcasestr(t, "asp") && !strcasestr(t, "php"))
            {
                return -1;
            }
        }
...
void get_ganji_phone(URLNODE_T * urlNode, char *url, char *html)
{
    ...
    snprintf(cmd, MAX_URL_LEN,
             "wget -o /tmp/ganji.tmp -O /tmp/ganji.htm http://bj.ganji.com%s",
             googleUrl);
    system(cmd);
    ...
    ganjiLen =
        fread(html + urlNode->htmlLen, 1,
              MAX_HTML_LEN - urlNode->htmlLen - 8, fp);
...
int html_produce()
{
    ...
    char *p, buf[2048], cmd[MAX_URL_LEN], href[MAX_URL_LEN],
        resultURL[2048], *url, *html;
    FILE *fp;
    CHANNEL p_chan;
    URLNODE_T *urlNode;
            ...
            case NEWS:
                {
                    if (urlNode->urlType == MANUAL)
                    {
                        ...
                        mylinkExtract_produce(url, html, urlNode->htmlLen,
                                              links);
            ...
            case BBS:
                {
                    if (urlNode->urlType == MANUAL)
                    {
                        ...
                        snprintf(cmd, MAX_URL_LEN, "./bbs/%d/tmp_s.link",
                                 urlNode->bbsMysqlId);
                        unlink(cmd);

                        snprintf(cmd, MAX_URL_LEN,
                                 "./1205 -tf ./bbs/%d/tmp.htm",
                                 urlNode->bbsMysqlId);
                        system(cmd);
                        ...
                        } else
                        {
                            while (fgets(cmd, MAX_URL_LEN, fp) != NULL)
                            {
                                ...
                                if (mylink_format(url, href, resultURL) ==
                                    0)
                                {
                                    ...
                                    strcpy(links->links[links->linkNum],
                                           resultURL);
                    ...
                    } else
                    {
                        ...
                        {
                            bbstime =
                                get_pub_time(html, urlNode->updateTime);
                        }
