...
char *memmem(const char *str1, const char *str1End, const char *str2,
             int str2Len)
{
    char *cp = (char *) str1;
    char *s1, *s2;
    ...
    if (n != 0 && m != 0)
    {
        ...
        for (i = 1; i < n; i++)
            for (j = 1; j < m; j++)
            {
                ...
                //Step 6                         
                d[j * n + i] =
                    minimum(d[(j - 1) * n + i] + 1, d[j * n + i - 1] + 1,
                            d[(j - 1) * n + i - 1] + cost);
            }
        distance = d[n * m - 1];
        free(d);
...
// 计算两个网页的相似度
int get_web_page_similarity(const char *characterA, int lengthA,
                            const char *characterB, int lengthB)
{
    ...
    if (similarity < 50)
    {
        // 剩余长度相差50%以上，不进行比较了
        similarity =
            (100 * sameLen + similarity * minLen) / (maxLen + sameLen);
        return similarity;
    }
    ...
    if (minLen < 4 * SIMILAR_SMALL_ELEMENT_LEN)
    {
        ...
        similarity =
            maxLen - levenshtein_distance(innerA, innerLenA, innerB,
                                          innerLenB);
    ...
    for (i = 0; i < sectionsA; i++)
    {
        ...
        } else
        {
            ...
            {
                searchEnd =
                    lastPosB + (curPosA - lastPosA) * 2 + elementLen;
                if (searchEnd > tailB)
                {
                    searchEnd = tailB;
                }
    ...
    for (i = 0; i < sectionsB; i++)
    {
        ...
        } else
        {
            ...
            {
                searchEnd =
                    lastPosA + (curPosB - lastPosB) * 2 + elementLen;
                if (searchEnd > tailA)
                {
                    searchEnd = tailA;
                }
    ...
    {
        similarity =
            ((100 * sameLen + similarity * innerLenA) / lengthA +
             (100 * sameLen + similarity * innerLenB) / lengthB) / 2;
    }
    return similarity;
}
