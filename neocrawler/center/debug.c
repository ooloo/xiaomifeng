...
int main(int argc, char **argv)
{
    ...
    if (neo_bdb_search(&key, dat) == 0)
    {
        ...
        sscanf(dat, "%d %d %u %u %u %u %u %u %u %u %u %u %u %s %s",
               ...
               &urlNode.statusCode,
               &urlNode.bbsMysqlId,
               &urlNode.linkPickInfo, &urlNode.htmlLen, url, ip);

        printf("----------------------------------------------------\n");
        printf("urlNode.insertTime	= %s",
               ctime((time_t *) & urlNode.insertTime));
        printf("urlNode.updateTime	= %s",
               ctime((time_t *) & urlNode.updateTime));
