 ...
 */
char *tree_visitor_for_link::get_attribute_value(html_tag_t * html_tag,
                                                 char *name)
{
 ...
 */
int tree_visitor_for_link::pre_visit(html_tag_t * html_tag, void *result,
                                     int min_font)
{
    ...
    if (html_tag->tag_type == TAG_PURETEXT
        && (link_data->in_anchor == 1 || link_data->in_option == 1))
    {
        /* copy text to the last link data. */
        snprintf(link_data->link[link_data->available - 1].text,
                 MAX_TEXT_LEN, "%s", html_tag->text);
    ...
    src = get_attribute_value(html_tag, (char*)"href");
    //no any link, javascript link, inner link.
    if (src == NULL || strncmp(src, "javascript:", 11) == 0
        || strncmp(src, "#", 1) == 0)
    {
        return VISIT_NORMAL;
    }
...
int tree_visitor_for_link::extract_link(link_t * link, int num)
{
    ...
    if (tree->base_url_ && strlen(tree->base_url_))
    {
        ...
        if (port && (atoi(port) != 80))
            sprintf(link_data.base_host, "http://%s:%s",
                    base_URL_.getHost(), port);
        else
