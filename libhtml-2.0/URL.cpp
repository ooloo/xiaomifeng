 ...
 */
URL::URL():url(NULL), scheme(NULL), host(NULL), port(NULL), path(NULL),
depth(0)
{
}
 ...
 */
URL::URL(const char *str):url(NULL), scheme(NULL), host(NULL), port(NULL),
path(NULL)
{
    setURL(str);
}
    ...
    if (*p == '/')
    {
        if (p[1] == '/')
        {
            ...
            *p = 0;             // Terminate access             
            ...
            if (p_ask
                && (((unsigned long) p_ask) < ((unsigned long) p)
                    || p == NULL))
            {
                int path_len;
