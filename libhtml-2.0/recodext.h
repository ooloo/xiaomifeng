...
#define REALLOC(Variable, Count, Type) \
  (Variable = (Type *) recode_realloc (outer, Variable,		\
   ...
   Variable)
    ...
    unsigned char *invert_table
        PARAMS((RECODE_OUTER, const unsigned char *));
    bool complete_pairs
        PARAMS((RECODE_OUTER, RECODE_STEP,
                const struct recode_known_pair *, unsigned, bool, bool));
...
#endif
                                               /* Global macros specifically for `recode'.  *//* Giving a name to the ASCII character assigned to position 0.  */
#define NUL '\0'
#if USE_HELPERS
