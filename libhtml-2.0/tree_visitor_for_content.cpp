 ...
 */
int tree_visitor_for_content::pre_visit(html_tag_t * html_tag,
...
//              content_data->visit_title = 1;
    ...
    } else if (html_tag->tag_type == TAG_PURETEXT)
    {
        ...
        if ((strstr(html_tag->text, "¡£") != NULL)
            || (strstr(html_tag->text, "£¬") != NULL))
        {
            src = html_tag->text;
        ...
        } else if (content_data->in_anchor == 1)
        {
            ...
            while (aaa != NULL)
            {
                    ...
                    && (aaa->value != NULL))
                {
                    ...
                    {
                        *content_data->filter_len +=
                            strlen(html_tag->text);
#ifdef _FTXT_PRINT_
 ...
 */
int tree_visitor_for_content::post_visit(html_tag_t * html_tag,
                                         void *result)
{
    content_data_t *content_data;
 ...
 */
int tree_visitor_for_content::extract_content(unsigned int *filter_len,
...
//      assert(size > 0);
    ...
    *content = '\0';
    ...
    if ((content_data.anchor_count > 600) ||
        ...
        ((content_data.pure_text_len < 3072)
         && (content_data.anchor_text_len * 10 /
             (content_data.pure_text_len + 1) > 9)))
    {
        tree->page_type = 0;
    }
/*
